...
{
	//all elements of a2 found
	if (n2 <= 0)
		return 1;

	//not enough elements left in a1
	if (n1 < n2)
		return 0;

	//if match not found continue to search for match
	if (*a1 != *a2)
		return countIncludes(a1 + 1, n1 - 1, a2, n2);

	//otherwise, continue current sequence + search for other possible sequences
	else
...
{
	//return if array is empty or has 1 element
	if (n <= 0 || n == 1)
		return;
	//split array
	int firstNotGreater, firstLess;
	split(a, n, *a, firstNotGreater, firstLess);
